...
def format_output_path(output_dir: str, nii_name: str, seg_name: str) -> str:
    ...
    """
    output_path = os.path.join(output_dir, f"{nii_name}_{seg_name}.nii.gz")
    return output_path
...
def main_conv(dicom_seg_path, nii_path, output_path):
    dicom_seg = read_seg(dicom_seg_path)  # (z, x, y)
    sitk_dcm_seg = sitk.ReadImage(dicom_seg_path)
    nii = read_nii(nii_path)  # (x, y, z)
    nii = nii[:, :, :, 0] #if nii.ndim == 4 else nii  # if 4d, take the first channel
    out_paths = []
